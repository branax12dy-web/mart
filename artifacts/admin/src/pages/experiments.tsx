                  ...
                  <TableRow key={exp.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="sm" onClick={() => setShowResults(exp.id)} title="View Results">
                          <BarChart3 className="w-4 h-4" />
                        </Button>
                        {exp.status === "active" && (
                          <Button variant="ghost" size="sm" onClick={() => statusMutation.mutate({ id: exp.id, status: "paused" })} title="Pause">
                            <Pause className="w-4 h-4" />
                          </Button>
                        )}
                        {exp.status === "paused" && (
                          <Button variant="ghost" size="sm" onClick={() => statusMutation.mutate({ id: exp.id, status: "active" })} title="Resume">
                            <Play className="w-4 h-4" />
                          </Button>
                        )}
                        {(exp.status === "active" || exp.status === "paused") && (
                          <Button variant="ghost" size="sm" onClick={() => statusMutation.mutate({ id: exp.id, status: "completed" })} title="Complete">

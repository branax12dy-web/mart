...
import { fc, CARD, CARD_HEADER, INPUT, SELECT, TEXTAREA, BTN_PRIMARY, BTN_SECONDARY, LABEL, PAGE, SECTION } from "../lib/ui";
...
export default function Products() {
    ...
    <div className={PAGE}>
      <Header pb="pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-white">{editProd ? "Edit Product" : "Add Product"}</h1>
      ...
      <div className={SECTION}>
        <div className={CARD + " p-4 space-y-3"}>
          ...
          {form.image && <div className="rounded-xl overflow-hidden h-32 bg-gray-100"><img src={form.image} alt="preview" className="w-full h-full object-cover" onError={e => (e.currentTarget.style.display="none")}/></div>}
        </div>
    ...
    <div className={PAGE}>
      <Header pb="pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-white">Bulk Add Products</h1>
  ...
  return (
    <div className={PAGE}>
      <Header pb="pb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setView("bulk")} className="h-9 px-3.5 bg-white/20 text-white text-xs font-bold rounded-xl android-press min-h-0">Bulk Add</button>
            <button onClick={() => setShowAdd(true)} className="h-9 px-3.5 bg-white text-orange-500 text-sm font-bold rounded-xl android-press min-h-0">+ Add</button>
          </div>
        </div>
      ...
      {/* Category Chips */}
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10" style={{ boxShadow: "0 2px 4px rgba(0,0,0,0.06)" }}>
          ...
          {categories.map(c => (
            <button key={c} onClick={() => setFilterCat(c)}
              className={`h-8 px-3.5 rounded-full text-xs font-bold whitespace-nowrap capitalize android-press min-h-0 flex-shrink-0 transition-all
                ${filterCat === c ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-600"}`}>
              {c}
            </button>
          ))}
        </div>
        ...
        {lowStock.length > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-2xl px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-500 mt-0.5">Edit products to update stock</p>
            </div>
          </div>
        )}

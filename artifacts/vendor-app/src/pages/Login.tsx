    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-orange-100 mt-1">AJKMart Business Partner</p>
        </div>
          ...
          {step === "phone" ? (
            <>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    autoFocus
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full h-12 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
              >
            ...
            <>
              <button onClick={() => { setStep("phone"); setError(""); }} className="text-orange-500 text-sm font-semibold mb-4 flex items-center gap-1">
                ← Back
              </button>
              ...
              {devOtp && <div className="bg-orange-50 border border-orange-200 rounded-lg px-3 py-2 mb-4 text-sm text-orange-700"><strong>Dev OTP:</strong> {devOtp}</div>}
              <div className="mb-4">
                <input
                  type="number" placeholder="Enter 6-digit OTP" value={otp}
                  onChange={e => setOtp(e.target.value)} onKeyDown={e => e.key === "Enter" && verifyOtp()}
                  className="w-full h-14 px-4 bg-gray-50 border border-gray-200 rounded-xl text-center text-2xl font-bold tracking-[0.3em] focus:outline-none focus:ring-2 focus:ring-orange-500"
              ...
              <button onClick={sendOtp} className="w-full mt-3 text-sm text-gray-500 hover:text-orange-500">Resend OTP</button>
            </>
          )}
        </div>
        <p className="text-center text-orange-100 text-xs mt-6">Only verified vendors can access this portal</p>
      </div>
    </div>
  );
}

...
import { fc, CARD, CARD_HEADER, INPUT, SELECT, TEXTAREA, BTN_PRIMARY, LABEL, PAGE, SECTION } from "../lib/ui";
    ...
    <div className={PAGE}>
      ...
      <Header pb="pb-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-white">My Store</h1>
            <p className="text-orange-100 text-sm mt-0.5">{user?.storeName || "Store settings"}</p>
          </div>
          <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${user?.storeIsOpen ? "bg-green-400/90 text-white" : "bg-red-400/90 text-white"}`}>
      ...
      <div className="bg-white border-b border-gray-100 flex sticky top-0 z-10" style={{ boxShadow: "0 2px 4px rgba(0,0,0,0.06)" }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key as any)}
            className={`flex-1 flex flex-col items-center py-2.5 text-[11px] font-bold border-b-2 transition-colors android-press min-h-0

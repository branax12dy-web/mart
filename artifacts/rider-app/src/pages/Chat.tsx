...
const BASE = `/api`;
function getToken() {
  try { return localStorage.getItem("ajkmart_rider_token") ?? ""; } catch { return ""; }
}
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const socket = io(window.location.origin, { path: "/api/socket.io", auth: { token: getToken() }, transports: ["polling", "websocket"] });
  ...
  const loadConversations = () => apiFetch("/communication/conversations").then(setConversations).catch(() => {});
  const loadRequests = () => apiFetch("/communication/requests?type=received").then(setRequests).catch(() => {});
  ...
  const sendMessage = async () => {
    ...
    setSending(true);
    try {
      const msg = await apiFetch(`/communication/conversations/${selectedConv.id}/messages`, { method: "POST", body: JSON.stringify({ content: input, messageType: "text" }) });
  ...
  const searchUser = async () => {
    ...
    try { setSearchResult(await apiFetch(`/communication/search/${searchId.toUpperCase()}`)); } catch { setSearchResult(null); }
  };
  ...
  const acceptRequest = async (id: string) => { await apiFetch(`/communication/requests/${id}/accept`, { method: "PATCH" }); loadRequests(); loadConversations(); };
  const rejectRequest = async (id: string) => { await apiFetch(`/communication/requests/${id}/reject`, { method: "PATCH" }); loadRequests(); };
  ...
  const endCall = useCallback(() => {
    ...
    setCallActive(false); setCallId(null); setCallTimer(0); setIncomingCall(null);
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      {selectedConv && (
        <div className="p-4 border-t bg-white">
          <div className="flex gap-2">
            <input value={input} onChange={e => { setInput(e.target.value); socketRef.current?.emit("comm:typing:start", { conversationId: selectedConv.id, userId: user?.id }); }} onBlur={() => socketRef.current?.emit("comm:typing:stop", { conversationId: selectedConv.id, userId: user?.id })} placeholder="Type a message..." className="flex-1 h-12 px-4 rounded-xl border outline-none" onKeyDown={e => e.key === "Enter" && sendMessage()} />

    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-gradient-to-br from-green-600 via-emerald-600 to-teal-700 px-5 pt-12 pb-8 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-green-200 text-sm mt-0.5">{T("incomePerformance")}</p>
        </div>
      </div>

      <div className="px-4 -mt-3 space-y-4">

        <div className="bg-white rounded-2xl shadow-sm p-5 relative overflow-hidden border border-green-100">
          ...
          <div className="relative">
            ...
            <p className="text-4xl font-extrabold text-green-600 mt-1">{formatCurrency(Number(user?.walletBalance) || 0)}</p>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gradient-to-br from-green-600 to-emerald-700 rounded-2xl p-5 text-white shadow-sm">
              ...
              <p className="text-xs text-green-300 mt-1">{riderKeepPct}% {T("deliveries").toLowerCase()}</p>
            </div>
            <div className="bg-emerald-50 rounded-2xl p-5 border border-emerald-100">
              ...
              <p className="text-xs text-emerald-500 mt-1">{T("completedLabel")}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 border border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="font-bold text-gray-800 text-sm flex items-center gap-1.5"><Target size={14} className="text-green-600"/> {T("dailyGoal")}</p>
              <p className="text-xs text-gray-400 mt-0.5">Target: {formatCurrency(dailyGoal)}/day</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-extrabold text-green-600">{todayPct}%</p>
              <p className="text-xs text-gray-400">{formatCurrency(data?.today?.earnings || 0)}</p>
            </div>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-3.5 overflow-hidden">
            <div
              className={`h-3.5 rounded-full transition-all duration-700 ${todayPct >= 100 ? "bg-green-500" : todayPct >= 60 ? "bg-emerald-400" : "bg-yellow-400"}`}
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 border border-gray-100">
          <p className="font-bold text-gray-800 text-sm mb-3.5 flex items-center gap-1.5"><BarChart2 size={14} className="text-blue-600"/> {T("performance")}</p>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-50 rounded-2xl p-4 text-center border border-blue-100">
              <p className="text-2xl font-extrabold text-blue-700">{totalDeliveries}</p>
              <p className="text-xs text-blue-500 font-semibold mt-1 flex items-center justify-center gap-1"><ClipboardList size={11}/> {T("totalDeliveries")}</p>
            </div>
            <div className="bg-purple-50 rounded-2xl p-4 text-center border border-purple-100">
              <p className="text-2xl font-extrabold text-purple-700">{formatCurrency(avgPerDelivery)}</p>
              <p className="text-xs text-purple-500 font-semibold mt-1 flex items-center justify-center gap-1"><TrendingUp size={11}/> {T("avgPerDelivery")}</p>
            </div>
            <div className="bg-orange-50 rounded-2xl p-4 text-center border border-orange-100">
              <p className="text-2xl font-extrabold text-orange-700">{formatCurrency(totalEarnings)}</p>
              <p className="text-xs text-orange-500 font-semibold mt-1 flex items-center justify-center gap-1"><CreditCard size={11}/> {T("allTimeEarned")}</p>
            </div>
            <div className="bg-green-50 rounded-2xl p-4 text-center border border-green-100">
              ...
              <p className="text-xs text-green-500 font-semibold mt-1">{ratingLabel}</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-start gap-2.5">
                ...
                <p className="text-xs text-green-700 leading-relaxed font-medium">{item}</p>
              </div>
            ))}
          </div>
        </div>

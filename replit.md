...
- **Authentication:** JWT-based authentication across all user roles (customer, rider, vendor). Supports multiple login methods including Phone OTP, Email OTP, Username/Password, and Email Registration (with verification email via nodemailer/SMTP). Includes role-specific registration (customer/rider/vendor) with CNIC validation, password strength rules, reCAPTCHA v3 middleware (fail-closed), OTP-based password reset with email delivery, TOTP 2FA (RFC 6238, encrypted at rest via AES-256-GCM), and per-role auth toggle enforcement via platform_settings (JSON format: `{"customer":"on","rider":"on","vendor":"on"}`). All auth toggle checks use `isAuthMethodEnabled()` for consistent parsing. User approval workflows for riders and vendors managed via the admin panel.
...
- `magicLinkTokensTable`: Stores magic link tokens for passwordless login (id, userId, tokenHash, expiresAt, usedAt, createdAt).
